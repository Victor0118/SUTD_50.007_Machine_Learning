...
private:
	...
	void viterbi(const vector<wstring> &adj, vector<int> &res) {
		...
		vector <int> fa[Num_y];
		for (int i = 0; i < Num_y; ++i) {
			pv[i] = log(transition_fixed(-1, i) * emission_fixed(adj[0], i));
		}
		for (int i = 1; i < n; ++i) {
			...
			for (int j = 0; j < Num_y; ++j) {
				...
				for (int k = 0; k < Num_y; ++k) {
					double cur = pu[k] + log(transition_fixed(k, j));
					if (cur > score) {
						score = cur;
						p = k;
					}
		...
		int fa_n = -1;
		for (int i = 0; i < Num_y; ++i) {
			double cur = pv[i] + log(transition_fixed(i, -2));
			if (cur > score_n) {
				score_n = cur;
				fa_n = i;
			}
	...
	void test(int which) {
		...
		string output = dataset[which] + "\\dev.p3.out";
		wifstream wfin(input.c_str());
